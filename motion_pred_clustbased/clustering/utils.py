...
def create_dirs(cfg):
    ts = str(datetime.datetime.now().strftime('%y-%m-%d_%a_%H:%M:%S:%f'))
    # main keys
    data_info = cfg["data"]
    clustering_info = cfg["clustering"]
    ds = data_info['dataset']
    ds_t = str(data_info['dataset_target'])
    final_ds = ds + f"_{ds_t}"
    
    algorithm = clustering_info["algorithm"] # algorithm type 
    algorithm = algorithm if algorithm != "k-means" else clustering_info[algorithm]["type"] + algorithm
    inp_type = data_info["inputs"]
    save_path = cfg['save']['path']
    assert inp_type in ["raw", "vect_dir", "polar"], f"{inp_type} not recognized"

    outputs_path = os.path.join(save_path, "outputs", "clustering", final_ds, algorithm, inp_type, ts)
    runs_path = os.path.join(save_path, "runs", "clustering", final_ds, algorithm, inp_type, ts)
    ...
    return outputs_path, runs_path


def save_clustering_object(check_dir, clust_obj, n_cl):
    """ saving clustering
    Args:
        check_dir (str): path to checkpoint directory
        clust_obj (object): clustering object
    """
    fn = f"clustering_{n_cl}"
    path = os.path.join(check_dir, fn +".pkl")
    with open(path,  "wb") as f:
        cPickle.dump(clust_obj, f)
        
        

def save_results(check_dir, metrics, n_cl):
    fn = f"clustering_{n_cl}"
    n_metrics = len(metrics[list(metrics.keys())[0]])
    results_line = []
    for i in range(n_metrics):
        for k, v in metrics.items():
            results_line.append(f"ID {i+1} -> {k}:{v[i]}\n")
        results_line[-1] += "\n\n"
    file = open(os.path.join(check_dir, fn+".txt"),"w")
